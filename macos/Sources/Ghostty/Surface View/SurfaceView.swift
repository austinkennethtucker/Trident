...
extension Ghostty {
    ...
    struct SurfaceSearchOverlay: View {
        ...
        private func overlay(in containerSize: CGSize) -> some View {
            clippedOverlay {
                HStack(spacing: 4) {
                    searchField
        ...
        private var searchField: some View {
            TextField("Search", text: $searchState.needle)
                ...
                .backport.onKeyPress(.return) { modifiers in
                    navigateSearch(modifiers.contains(.shift) ? "navigate_search:previous" : "navigate_search:next")
                    return .handled
                }
        }
        ...
        @ViewBuilder
        private func clippedOverlay<Content: View>(@ViewBuilder content: () -> Content) -> some View {
#if compiler(>=6.2)
            if #available(iOS 26.0, macOS 26.0, *) {
                content()
                    .clipShape(ConcentricRectangle(corners: .concentric(minimum: 8), isUniform: true))
            } else {
                content()
